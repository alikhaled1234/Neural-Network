...
double matc<T>::rands()
{
	...
	//std::cout << ((double)(rand() % 5) * sign) + ((double)(rand() % 100) * (sign / 100.0)) << '\n';
	//return ((double)(rand() % 5) * sign) + ((double)(rand() % 100) * (sign / 10.0));
...
template<typename T>
double matc<T>::xavierInit(double in_size, double out_size) {
	//std::cout << getNormalRandom(0, 0.17) / sqrt(pow(out_size, 1)) << '\n';

	return getNormalRandom(0, 0.17) / sqrt(out_size);
	//return sqrt(2.0 / (in_size + out_size)) * ((double)rand() / RAND_MAX - 0.5);
}

template<typename T>
double matc<T>::getNormalRandom(double mean, double standardDeviation) {
	std::random_device rd;
	std::mt19937 gen(rd());
	std::normal_distribution<> dis(mean, standardDeviation);
	return dis(gen);
}

template<typename T>
void matc<T>::resize(int rows, int cols, int random) {
	...
	if (random == 1) {
		...
		for (int i = 0; i < rows * cols; i++) {
			matrix[i] = xavierInit(rows - 1, cols);
