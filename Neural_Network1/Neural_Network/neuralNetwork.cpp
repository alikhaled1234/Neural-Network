...
neuralNetwork::neuralNetwork(int layerNum, int layerNodes[]) {
	...
	for (int i = 0; i < layerNum; i++) {
		...
		if(i != 0) {
			...
			weights[i].resize(layerNodes[i - 1] + 1, layerNodes[i], 2);
...
void neuralNetwork::rehsapeLayer(int layer, int newSize) {

	numofLayerNodes[layer] = newSize;

	if (layer != numOfLayers - 1) {
		weights[layer + 1].resize(numofLayerNodes[layer] + 1, numofLayerNodes[layer + 1], 2);
		dC_dW[layer + 1].resize(numofLayerNodes[layer] + 1, numofLayerNodes[layer + 1], 0);
	}
	else if (layer != 0) {
		weights[layer].resize(numofLayerNodes[layer - 1] + 1, numofLayerNodes[layer], 2);

		dC_dW[layer].resize(numofLayerNodes[layer - 1] + 1, numofLayerNodes[layer], 0);
	}

	if (layer == numOfLayers - 1) {
		activatedNodes[layer].resize(1, numofLayerNodes[layer], 0);
		activationFunc[layer] = NONE;
	}
	else {
		activatedNodes[layer].resize(1, numofLayerNodes[layer], 0);
		activatedNodes[layer].concat(matc<double>(), true);
	}

	notActivatedNodes[layer].resize(1, numofLayerNodes[layer], 1);
	dC_dZ[layer].resize(1, numofLayerNodes[layer] + 1, 0);
}
